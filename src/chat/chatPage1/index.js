...
import { ScrollDiv } from "../chatPage6/styled";
...
export const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 1)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    try {
      setBut(true)
      localStorage.setItem("2", new Date().toLocaleTimeString().slice(0, 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv ref={divRef}>
        ...
        {(parseInt(localStorage.getItem("count")) > 1) ? <DoneSpan>task canceled</DoneSpan>
          :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 1) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
