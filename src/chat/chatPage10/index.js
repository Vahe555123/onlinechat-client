...
import { ScrollDiv } from "../chatPage1/styled";

export const ChatPage10 = () => {
  ...
  const sendMessage = async () => {
    ...
    if (localStorage.getItem("question") && divRef.current) {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 10)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    setBut(true)
    localStorage.setItem("11" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv>
        ...
        {(parseInt(localStorage.getItem("count")) > 10) ? <DoneSpan>task canceled</DoneSpan>
          :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 10) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
