...
export const ChatPage8 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 8)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    setBut(true)
    localStorage.setItem("9" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv>
        ...
        {(parseInt(localStorage.getItem("count")) > 8) ? <DoneSpan>task canceled</DoneSpan>
          :
