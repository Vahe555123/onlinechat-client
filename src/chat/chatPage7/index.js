...
export const ChatPage7 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 7)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    setBut(true)
    localStorage.setItem("8" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv>
        ...
        {(parseInt(localStorage.getItem("count")) > 7) ? <DoneSpan>task canceled</DoneSpan>
          :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 7) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
