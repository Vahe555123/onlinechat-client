...
export const ChatPage4 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 4)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async() => {
    try {
      setBut(true)
      localStorage.setItem("5" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <SpanZag>
                    ...
                    {(parseInt(localStorage.getItem("count")) > 4) ? <DoneSpan>task canceled</DoneSpan>
                    :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 4) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
