...
export const ChatPage5 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 5)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async() => {
    setBut(true)
    localStorage.setItem("6" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <SpanZag>
                    ...
                    {(parseInt(localStorage.getItem("count")) > 5) ? <DoneSpan>task canceled</DoneSpan>
                    :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 5) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
