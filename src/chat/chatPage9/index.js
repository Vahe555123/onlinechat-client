...
export const ChatPage9 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 9)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    setBut(true)
    localStorage.setItem("10" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv>
        ...
        {(parseInt(localStorage.getItem("count")) > 9) ? <DoneSpan>task canceled</DoneSpan>
          :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 8) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
