...
export const ChatPage6 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 6)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    }
  }, [myMessage]);
  ...
  const changeCount = async () => {
    try {
      setBut(true)
      localStorage.setItem("7" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <ScrollDiv ref={divRef}>
        ...
        {(parseInt(localStorage.getItem("count")) > 6) ? <DoneSpan>task canceled</DoneSpan>
          :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 6) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
