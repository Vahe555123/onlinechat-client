...
export const ChatPage3 = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("question") && divRef.current && !(parseInt(localStorage.getItem("count")) > 3)) {
      divRef.current.scrollTop = divRef.current.scrollHeight;
    } 
  }, [myMessage]);
  ...
  const changeCount = async() => {
    try {
      localStorage.setItem("4" , new Date().toLocaleTimeString().slice(0 , 5))
  ...
  return (
    <ChatDiv>
      ...
      <SpanZag>
                    ...
                    {(parseInt(localStorage.getItem("count")) > 3) ? <DoneSpan>task canceled</DoneSpan>
                    :
      ...
      {localStorage.getItem("question") && !(parseInt(localStorage.getItem("count")) > 3) ? (
        <ChatDivs>
          <MessageDiv />
          <InputDiv>
            <InputChat
